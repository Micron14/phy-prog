...
\begin{itemize}
    ...
    \item \textbf{Uso di \texttt{std::generate\_n}: }È stato implementato l'algoritmo di generazione \texttt{std::generate\_n} all'interno di \texttt{std::vector<Boid> entity\_gen}, al fine di evitare il precedente ciclo \texttt{for}, quindi rendere più espressivo e ottimizzato il codice.
...
\begin{itemize}
    ...
    \item \textbf{Integrazione fisica e Generazione (\texttt{simulation}):}
    \begin{itemize}
        \item \textit{Ciclo di simulazione:} Verificata l'evoluzione temporale $\Delta t$ (\texttt{update\_physics}), delle interazioni dinamiche con il mouse (attrazione e repulsione), della reazione di fuga dai predatori e del corretto comportamento dei confini.
        \item \textit{Generazione entità:} Controllo del posizionamento casuale degli agenti all'intero della finestra (\texttt{entity\_gen}) e gestione delle eccezioni (\texttt{std::runtime\_error}) con parametri di popolazione negativi o nulli ($N \le 0$).
    \end{itemize}
...
\end{itemize}
\subsection{Costrutti non trattati a lezione}
Nello sviluppo del codice relativo al progetto sono stati implementati degli algoritmi non trattati a lezione. Di seguito, in tabella (\ref{costrutti}) riportiamo i principali costrutti utilizzati.
\begin{table}[H]
    \centering
    \caption{Algoritmi non trattati a lezione}
    \label{costrutti}
\begin{tblr}{
  colspec = {Q[c,m] X[m, c]},
  hlines, vlines}
  \textbf{Algoritmo} & \textbf{caratteristiche} \\ \hline
  \texttt{std::move} & Nella funzione \texttt{update\_physics}, l'aggiornamento dello stato dello sciame segue uno schema di double buffering. Per evitare dipendenze dall'ordine di iterazione (in cui il calcolo delle forze per il boid $i$-esimo verrebbe influenzato dalle posizioni già aggiornate dei boid precedenti), i nuovi stati vengono accumulati in due vettori locali temporanei, \texttt{next\_flock} e \texttt{next\_kettle}. Al termine del ciclo, occorre aggiornare i vettori di stato principali (\texttt{flock} e \texttt{kettle}). Invece di eseguire una copia elemento per elemento, si ricorre a \texttt{std::move}. Viene creato un oggetto nuovo ri utilizzando le risorse di proprietà di un altro. \\
\end{tblr}
\end{table}
...
\begin{tblr}{
  colspec = {Q[c,m] X[m, c]},
  hlines, vlines}
  \texttt{std::transform\_reduce} & È stato preferito l'utilizzo dell'algoritmo \texttt{std::transform\_reduce} della Standard Library al fine di utilizzare del codice ben implementato e ottimizzato. L'algoritmo combina in un'unica iterazione la mappatura di ogni elemento (fase di transform) e l'accumulazione dei risultati (fase di reduce).\\
  \texttt{std::cin.clear} e \texttt{std::cin.ignore} & Se il flusso \texttt{std::cin} entra in uno stato di errore, tutte le successive operazioni di lettura falliscono silenziosamente. La funzione \texttt{clear\_cin\_buffer()} definita in \texttt{simconfig.cpp} è costrutita (attraverso l'utilizzo di \texttt{std::cin.clear} e \texttt{std::cin.ignore}) per ripristinare lo stato dello stream e svuotare completamente il buffer residuo, garantendo che le successive richieste di input funzionino correttamente e senza blocchi.\\
  \texttt{std::plus} & Nell'algoritmo \texttt{std::transform\_reduce}, serve a specificare l'operazione di riduzione, ossia come combinare tra loro i valori estratti da ciascun \texttt{Boid}.\\
  \texttt{std::vector::reserve} & L'istruzione \texttt{std::vector::reserve(n)} alloca preventivamente la memoria dinamica nello \textit{heap} per contenere almeno $n$ elementi, modificando la capacità del vettore senza alterarne la dimensione (\texttt{size()}, che rimane $0$). \\
\end{tblr}
\end{table}
...
Di seguito riportiamo l'indirizzo della repository: \url{https://github.com/Micron14/phy-prog}.
