...
\begin{itemize}
    ...
    \item \textbf{Uso di this e rimozione di self\_index: }Sono stati implementati dei range-for loop nelle funzioni di aggiornamento della simulazione. È stata modificata la logica di riconoscimento dello stesso boid attraverso la distanza (uguale a zero).
    \item \textbf{Uso appropriato di static: }Abbiamo modificato l'utilizzo di \texttt{static} nel codice, limitandolo alle variabili che ne necessitavano l'uso (al fine di rendere più efficiente il codice).
    \item \textbf{Modifiche relative alla relazione: }Dati i cambiamenti del codice abbiamo provveduto a perfezionare la descrizione di quest ultimo nella relazione da consegnare. Abbiamo inoltre aggiunto nuove spiegazioni in quest'ultima relative alle configurazioni iniziali della simulazione. 
...
Per inserire un elemento di disturbo nello stormo, il programma prevede la presenza dei cacciatori (\textit{hunters}). Rispetto ai boids ordinari, gli \textit{hunters} hanno parametri diversi: sono caratterizzati da dimensioni geometriche maggiori e si muovono a una velocità massima più elevata.

Il loro comportamento segue una logica specifica tramite l'applicazione di forze centrali con la funzione \texttt{apply\_force}:
\begin{itemize}
    ...
    \item \textbf{Forza Inerziale di Propulsione:} Ciascun cacciatore viene spinto costantemente nella direzione in cui si sta già muovendo. Il programma calcola una forza diretta verso un punto proiettato in avanti nel tempo lungo la sua traiettoria ($\vec{x}_{\text{hunter}} + 4\vec{v}_{\text{hunter}}$), simulando l'inerzia del movimento.
...
I boids avvertono il pericolo del predatore: non appena un \textit{hunter} entra nel loro raggio di influenza, i boids attivano una regola di fuga che ha la priorità assoluta su tutte le altre, spingendoli nella direzione opposta con una forza repulsiva a corto raggio.

\section{Scelte Progettuali e Ripartizione del Codice}
\label{sec:progettazione}
Il codice è scritto in C++ seguendo la programmazione a oggetti ed è diviso in diversi file separati per mantenere il lavoro ordinato e modulare. Abbiamo inoltre impiegato l'utilizzo di diversi namespace in modo da evitare conflitti di nomi.

\begin{itemize}
    \item \textbf{\texttt{vector2d.hpp}} (\texttt{namespace phy}): contiene le definizioni dell'interfaccia per la struttura dati \texttt{Vector2D}, progettata per la gestione di vettori nello spazio bidimensionale $\mathbb{R}^2$.
    \item \textbf{\texttt{vector2d.cpp}} (\texttt{namespace phy}): contiene le definizioni delle funzioni della struttura dati dichiarata nell'header \texttt{vector2d.hpp}. Inoltre sono presenti gli overload degli operatori dichiarati in esso.
    \item \textbf{\texttt{boid.hpp}} (\texttt{namespace boids}): definisce la classe \texttt{Boid}, che modella l'agente fondamentale della simulazione. Questa classe incapsula lo stato fisico dell'entità (posizione e velocità) e i metodi necessari per implementare il comportamento collettivo basato sul modello di Reynolds (separazione, allineamento e coesione).
    \item \textbf{\texttt{boid.cpp}} (\texttt{namespace boids}): implementa la logica comportamentale e cinetica della classe \texttt{Boid}. Basandosi sul modello di Reynolds, il codice traduce le interazioni locali tra le entità in vettori di forza, determinando il movimento fluido e collettivo tipico di uno stormo.
    \item \textbf{\texttt{statistics.hpp}} (\texttt{namespace stats}): contiene le dichiarazioni dell'interfaccia per il modulo di analisi statistica, progettato per elaborare i dati relativi a un gruppo di oggetti di tipo \texttt{Boid} (tipicamente rappresentanti una simulazione di stormo).
    \item \textbf{\texttt{statistics.cpp}} (\texttt{namespace stats}): fornisce l'implementazione logica delle funzioni di analisi statistica per lo stormo di Boid. Utilizza algoritmi della libreria standard C++ (\texttt{<algorithm>}, \texttt{<numeric>}) per elaborare in modo efficiente le proprietà del vettore flock.
    \item \textbf{\texttt{simconfig.hpp}} (\texttt{namespace boids}): definisce la struttura dati \texttt{SimConfig}, che centralizza tutti i parametri di configurazione necessari al motore di simulazione. Tale approccio permette di separare la logica dei parametri dai componenti dinamici (i \textit{Boid}), facilitando la gestione delle impostazioni sia globali che specifiche per eventuali sottogruppi (es. predatori o \textit{kettle}). Abbiamo implementato questo \textit{header file}, incluso ad esempio in \texttt{boid.cpp} e in \texttt{boid.hpp}, in modo da poter dichiarare e definire delle funzioni che accettassero in input una singola struct, al posto di svariati parametri. Sono state dichiarate delle funzioni necessarie alla configurazione della simulazione, qualora si volessero utilizzare dei parametri da passare attraverso \texttt{std::cin}.
    \item \textbf{\texttt{simconfig.cpp}} (\texttt{namespace boids}): implementa la logica di inizializzazione e configurazione dinamica dei parametri di simulazione. Gestisce l'interazione con l'utente tramite standard input per definire le caratteristiche dello stormo e dei predatori (\textit{hunter}), includendo meccanismi di validazione dei dati in ingresso.
    \item \textbf{\texttt{simulation.hpp}} (\texttt{namespace sim}): contiene le dichiarazioni dell'interfaccia per il modulo di aggiornamento fisico della simulazione. In esso è presente la dichiarazione della funzione \texttt{update\_physics}, che gestisce l'evoluzione temporale dello stormo (\textit{flock}) e di eventuali sotto-gruppi (\textit{kettle}), applicando le regole di comportamento (\textit{flocking}) basate sulle configurazioni (\texttt{SimConfig}) e sugli input dell'utente (posizione del mouse e stati dei tasti).
    \item \textbf{\texttt{simulation.cpp}} (\texttt{namespace sim}): implementa la logica di aggiornamento temporale della simulazione (\texttt{update\_physics}). Esso funge da motore principale, coordinando il calcolo delle forze, l'applicazione delle regole di comportamento (\textit{flocking}) e la gestione dei vincoli spaziali per le entità (\textit{Boid}) presenti sia nello stormo principale che nel sotto-gruppo dei predatori (\textit{kettle}).
...
\end{itemize}
Nell'archivio inviato sono presenti file come \texttt{CMakeLists.txt}, \texttt{.clang-format}, \texttt{doctest.h}, \texttt{arial.ttf} utilizati nello sviluppo del progetto.
...
\end{itemize}
Indipendentemente dalla modalità scelta, il sistema richiede esplicitamente l'inserimento del \textbf{numero di cacciatori (\textit{hunters})} da generare nella mappa. Questo permette di testare la risposta dinamica dello stormo a diversi livelli di pressione predatoria, osservando l'impatto sulla stabilità globale.
...
Nella presente architettura di simulazione, la generazione sia degli stormi di Boid (\textit{flocks}) che dei gruppi di predatori (\text{kettle}) è affidata a una funzione di utilità unica e generica: \texttt{entity\_gen}. Questo approccio di programmazione modulare permette di gestire la creazione di entità diverse utilizzando un'unica logica di generazione, garantendo coerenza e manutenibilità del codice. La funzione riceve in input un oggetto di tipo \texttt{boids::SimConfig} contenente tutti i parametri necessari per configurare l'entità. La funzione esegue un controllo sugli input: per gli stormi di \textit{boids} viene richiesto un numero di entità strettamente positivo, mentre per gli \textit{hunters} un numero non negativo. In caso di parametri non validi, il sistema solleva un'eccezione di tipo \texttt{std::runtime\_error}, prevenendo errori di esecuzione.
Utilizzando un motore di numeri casuali \texttt{std::default\_random\_engine} la funzione distribuisce le entità nello spazio $\mathbb{R}^2$. Le coordinate ($x$, $y$) sono calcolate entro i bordi definiti dalla configurazione, applicando un margine di sicurezza per evitare la generazione di boid sui confini. Ogni entità viene inizializzata con un vettore velocità ($v_x$,$v_y$) casuale, compreso in un intervallo che garantisce movimento iniziale dello stormo. A questo punto la funzione controlla il valore booleano \texttt{is\_hunter} presente nella configurazione: se esso ha valore \texttt{true}, vengono applicati i limiti di velocità specifici del predatore.

\subsubsection{Regole di volo}
Sono state implementate le regole di volo presentate in Sezione \ref{sec:modellofisico}:
\begin{itemize}
    \item \texttt{compute\_separation}: impedisce ai boids di sovrapporsi o scontrarsi. Per ogni vicino all'interno del \texttt{separarion\_radius} calcola un vettore repulsivo che allontana il boid corrente. L'intensità della spinta è regolata attraverso il \texttt{separation\_factor}.
...
L'evoluzione temporale del sistema adotta un approccio ad aggiornamento sincrono delle coordinate. All'interno della funzione \texttt{update\_physics}, lo stato dello stormo e del kettle al tempo $t$ viene preservato mediante una copia profonda in un vettore ausiliario (\texttt{next\_flock}) e (\texttt{next\_kettle}), sul quale si calcolano e applicano le forze. Successivamente, la riassegnazione efficiente avviene sfruttando la semantica di spostamento tramite \texttt{std::move(next\_flock)} e \texttt{std::move(next\_kettle)}. Ciò elimina asimmetrie temporali fittizie dovute all'ordine sequenziale dei cicli \texttt{for}.

Il modello standard è arricchito da due elementi:
\begin{enumerate}
    \item \textbf{Agenti Predatori (Hunters):} Il codice istanzia una flotta di \textit{hunters} cinematica e strutturalmente distinta (\texttt{kettle}), configurata con velocità di punta superiori. Essi inseguono il centro di massa dei boid definiti \textit{vicini} secondo le logiche descritte nella Sezione~\ref{sec:modellofisico}. A seconda del parametro \texttt{is\_bounded}, vengono applicati confini rigidi o toroidali. Atraverso il metodo \texttt{update} vengono integrate le forze nel vettore velocità determinando lo spostamento finale.
    \item \textbf{Boids}: Il metodo \texttt{update} calcola le forze di separazione, allineamento e coesione rispetto agli altri componenti dello stormo. Si applica una forza repulsiva verso la posizione di ogni \textit{hunter} presente nel vettore \texttt{kettle}, simulando l'istinto di fuga dei boids. Analogamente ai predatori, vengono applicati i vincoli di bordo per mantenere lo stormo nell'area definita.
    \item \textbf{Interattività Utente:} L'utente può perturbare il sistema in tempo reale tramite input asincroni del mouse. La pressione del tasto destro genera una forza attrattiva verso le coordinate del cursore, mentre il tasto sinistro simula una forza repulsiva (effetto "spavento").
\end{enumerate}

\subsubsection{Generazione dei grafici attraverso SFML}
Il modulo di rendering grafico è stato sviluppato, come anticipato, nel file \texttt{render.cpp}, che si basa sull'aggiornamento ciclico di primitive grafiche all'interno di una \texttt{sf::RenderWindow}. Il file si occupa di due tasks:
\begin{enumerate}
...
\end{lstlisting}
per avviare la simulazone in modalità Debug, oppure si avvia in modalità Release:
\begin{lstlisting}
...
\begin{enumerate}
    ...
    \item \textbf{Output di Diagnostica da Terminale:} Lo standard output (\texttt{std::cout}) stampa continuativamente ad ogni ciclo le grandezze fisiche medie e le deviazioni standard dello stormo nel seguente formato:
\begin{lstlisting}
...
\textbf{Gestione del Progetto (GitHub):}
